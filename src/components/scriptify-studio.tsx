...
const STORAGE_KEY_INFLUENCERS = 'scriptify_influencers_db';
const STORAGE_KEY_SCENES = 'scriptify_scenes_db';
...
export default function ScriptifyStudio() {
    ...
    // Load data from localStorage when component mounts on the client
    useEffect(() => {
        setHasMounted(true);
        ...
        try {
            const influencersData = localStorage.getItem(STORAGE_KEY_INFLUENCERS);
            if (influencersData) {
                setGalleryInfluencers(JSON.parse(influencersData));
            }
        } catch (e) {
            console.error("Failed to parse influencers from localStorage:", e);
        }

        try {
            const scenesData = localStorage.getItem(STORAGE_KEY_SCENES);
            if (scenesData) {
    ...
    useEffect(() => {
        if (hasMounted) {
            localStorage.setItem(STORAGE_KEY_INFLUENCERS, JSON.stringify(galleryInfluencers));
        }
